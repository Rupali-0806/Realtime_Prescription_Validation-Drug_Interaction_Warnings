#!/usr/bin/env python3
"""
Zaura Health - EC2 Optimized Flask Application
Drug Interaction Prediction API optimized for AWS Free Tier t2.micro
"""
...
def predict_drug_combinations(drugs: List[str], dosage: Optional[float] = None) -> Dict:
    """
    Memory-optimized prediction function for t2.micro
    """
    try:
        if len(drugs) < 2:
...
if __name__ == '__main__':
    ...
    logger.info("Loading Zaura Health Model for EC2...")
    
    if not load_model():
        logger.error("Failed to load model. Please ensure model files exist.")
        sys.exit(1)
    
    logger.info("✓ Zaura Health API ready on EC2!")
